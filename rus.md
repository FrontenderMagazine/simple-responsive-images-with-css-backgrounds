...
Всё же у этого подхода есть некоторые ограничения, и в некоторых случаях он 
...
Во многих случаях вся отзывчивость дизайна заканчивается именно 
на таких простых изменениях в 
расположении контента и его презентации. Если внимание уделено 
...
влияющих на удобство пользователя, в том числе с учётом изображений. 
Вот пример из жизни: взгляните на вебсайт [contfont.net][6]. Этот 
одностраничный сайт состоит из одного изображения и типичного набора HTML, 
...
однако обходится не дёшево: при разрешении 1940 × 1229 пикселей изображение 
...
создаёт не только теоретические неудобства — дополнительный вес значительно 
...
оптимально эффективный способ различать устройства. Мы получаем ответ на два 
важных вопроса: какой размер экрана на устройстве пользователя? Поддерживаются 
ли изображения, оптимизированные под ретину?

Эту информацию мы получаем благодаря CSS-*медиазапросам*. 
...
как этот способо будет выглядеть с привязкой к размеру экрана (код для ретины 
...
хоть оно и не будет отображаено. К сожалению, столь логичный подход не 
...
просмотра на реально существующем устройстве составляет 320 пикселей у iPhone без ретины в портретной ориентации. (Устройства с меньшей областью 
...
соответствующих изображений. Для дисплеев с ретиной нужно создать дополнительный 
...
можно сделать доступными для пользователей с дефектами зрения. Для этого 
...
от контекста пользователя. (Для краткости в следующем примере опущены 
вендорные префиксы).
...
нашему пользователю реальных денег за использование безпроводного подключения. 
