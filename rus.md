# Простой способ сделать изображение отзывчивым с помощью фоновых изображений в CSS

**Примечание редактора Smashing Magazine:** *В этой статье описано одно из 
...
том числе со следующими: [«Как избежать лишних загрузок для отзывчивых 
...
элемент `<picture>`, а также применении таких приёмов на стороне сервера как 
Отзывчивый вебдизайн + Серверные компоненты (RESS), не удивительно если у вас 
сложилось впечатление что на сегодняшний день простые статические вебсайты не 
...
У этого подхода всё же есть некоторые ограничения и в некоторых случаях он 
...
**В этой статье мы рассмотрим подход с использованием фоновых изображений в CSS 
в несколько этапов:**

1. Сначала мы рассмотрим задачи и требования, которые возникают при создании 
...
формируется отзывчивое изображение.
...
таблицей стилей и HTML-разметкой вашего вебсайта. Если для него используется 
система управления контентом (CMS), у вас может не быть достаточного доступа к 
управлению этими составляющими сайта. 

## Потребность в отзывчивых изображениях

Отзывчивые изображения - это важная составляющая отзывчивого вебдизайна, подхода 
к дизайну предложенного [Итаном Маркоттом (Ethan Marcotte)][5] для 
удовлетворения требований мобильных устройств, популярность которых в сети 
потрясающа. Итан признал что прежняя практика создания отдельных вебсайтов для 
разных типов устройств была не в состоянии справиться с требованиями огромного 
разнообразия устройств, доступных интернет-пользователям для доступа к 
разработанным нами вебсайтам.

Отзывчивый дизайн предлагает абсолютно иной подход: создание одного вебсайта, 
который распознаёт окружающий контекст и подстраивается под него. Например, если 
для просмотра вебсайта пользователь использует настольный компьютер с 
широкоформатным экраном, пусть содержимое вебсайта будет разделено на несколько 
колонок. С другой стороны, на смартфоне контент этого вебсайта должен быть 
переупорядочен таким образом, чтобы помещаться в одну колонку. 

Во многих случаях вся отзывчивость дизайна именно на таких простых изменениях в 
расположении контента и его презентации заканчивается. Если внимание уделено 
только этим аспектам, цель отзывчивого дизайна нельзя считать достигнутой и 
потребности наших пользователей не удовлетворены полностью.

Чтобы полностью соответствовать контексту, в котором пользователь просматривает 
вебсайт, дизайн этого вебсайта должен быть разработан с учётом всех аспектов, 
влияющих на опыт пользователя, в чье число входят и изображения. Вот пример из 
жизни, взгляните на вебсайт [contfont.net][6]. Этот одностраничный сайт состоит 
из одного изображения и типичного набора HTML, таблицы стилей, шрифтов и файлов 
JavaScript.
...
На компьютерах с большим разрешением экрана вебсайт выглядит просто шикарно 
благодаря большому разрешению главного изображения. Загрузка этого изображения, 
однако не обходится дёшево: при разрешении 1940 × 1229 пикселей изображение 
после сжатия весит 446 Кб. 

Теоретически можно использовать одно и то же изображение для всех пользователей 
без ущерба для визуального впечатления. Веб-браузеры на небольших устройствах 
могут подстроить его размер под размер экрана. Однако при таком подходе общий 
опыт пользователя при просмотре вебсайта сильно бы ухудшился, хоть визуальное 
представление вебсайта и остаётся неизменным.
...
Что это значит для пользователя, который просматривает вебсайт на, скажем, 
смартфоне старого поколения? Для этого пользователя изображение с разрешением 
290 × 183 пикселей и размером 18 Кб будет выглядеть идентично его большой версии. 
Если не применять отзывчивый подход к изображениям, **пользователю придётся 
скачивать дополнительные и абсолютно ненужные 429 Кб данных**. И это излишество 
не только теоретически, дополнительный вес значительно влияет на время загрузки 
вебсайта. Пользователь смартфона может просматривать вебсайт используя обычное 
подключение через 3G со скоростью 2 Мб/с. Если не использовать отзывчивое 
изображение, время загрузки сайта увеличивается от 1,3 секунд до 3 секунд, 
больше чем вдвое!
...
изображение с таким разрешением, какое может быть реально отображено на 
устройстве**.

## Определение пользовательского контекста

Поскольку наши вебсайты должны подстраиваться под контекст пользователя, нам 
нужен способ определения этого контекста. На сегодняшний день CSS предлагает 
оптимально эффективный способ различения устройств. Мы получаем ответ на два 
важных вопроса: какой размер экрана на устройстве пользователя? Поддерживаются 
ли изображения оптимизированные под дисплеи Retina?

Эту информацию мы получаем благодаря такому инструменту CSS как *медиазапросы*. 
Медиазапросы позволяют прописать набор стилевых свойств, которые будут 
...
большое их количество, например, `braille` (тактильные устройства, на системе 
Брайля), `speech` (речевые синтезаторы), `tty` (устройства, поддерживающие 
только моноширинные шрифты, такие как телетайпы) и `tv` (устройства вроде 
...
достаточно информации чтобы подобрать подходящее отзывчивое изображение.
...
(пока что не будем рассматривать наличие дисплея Retina). В таблице стилей 
...
в 1440 пикселей. Здесь однако есть маленький нюанс. Под размером экрана следует 
...
различных окантовок вроде полос прокрутки. 

В общем-то ширина окна браузера наверняка будет меньше 1440 пикселей, разве что 
пользователь переключился на полноэкранный режим просмотра. Нам стоит это учесть 
в таблице стилей. 1200 пикселей наверное будет ближе к истине.
...
минимальная ширина экрана, при которой стиль должен применяться. Оператор `and`, 
соединяющий эти части, указывает на то, что оба условия должны быть истинны 
чтобы стиль был применён.
...
устройства. Браузеры на смартфонах всегда работают в полноэкранном режиме. 

С помощью условий `min-width` и `max-width` можно легко определить ширину 
устройства пользователя. К сожалению, с определением дисплея Retina не всё так 
просто. Разные браузеры используют разный синтаксис для этого параметра, потому 
следует использовать вендорные префиксы. Ещё ложка дёгтя: в синтаксисе многих 
версий Firefox присутствует ошибка, так что нам придётся использовать и 
нормальный и «дефектный» синтаксис для браузеров от Mozilla. На данный момент 
для дисплеев с качеством Retina рекомендуется использовать медиазапросы такого 
вида:
...
дюйм) и мы сможем отказаться от вендорных префиксов в таблице стилей.

## Свойство CSS `background-image`

Поскольку CSS позволяет достоверно определить контекст пользователя, может 
показаться что обеспечить поддержку отзывчивых изображений легко. Логично вроде 
бы задать `display: none` для изображений, которые не должны загружаться. Вот 
как такой подход будет выглядеть с привязкой к размеру экрана (код для Retina 
пропустим в целях лаконичности):
...
размера экрана, однако свойство `display: none` не предотвращает загрузку 
скрытых изображений. Браузеры на смартфонах будут скачивать большое изображение, 
хоть оно и не будет отображаться. К сожалению, столь логичный подход не 
удовлетворяет главную задачу отзывчивых изображений.
...
предоставляет. Затем он проводит разбор стилей CSS и обнаруживает что одно из 
...
невозможным, но оказывается есть хитрый способ использовать только CSS для 
изображений с помощью свойства `background-image`. Вот как это делается:

1. В разметку HTML не помещайте никакие запросы на файлы изображений. Вместо 
этого используйте пустые элементы `<div>` или `<span>`.
2. Привяжите изображение к пустому `<div>` или `<span>` через свойство 
`background-image`.
3. Используйте медиазапросы чтобы изменять изображение в зависимости от размера 
и разрешения экрана.

Следуя этим правилам мы можем подкорректировать приведенный выше пример чтобы 
...
Дополнительный код будет стандартным CSS без каких-либо хитростей.

## Создание отзывчивых изображений

Теперь, когда мы разобрались как использовать отзывчивые изображения, следующий 
шаг - собственно создать разные варианты изображения. Исходное изображение 
...
настройках самое низкое качество изображения, которое будет приемлемым. 
Пользователи компьютеров с экранами с высоким разрешением оценят быструю 
загрузку сайта. Если изображение в формате JPEG, при его экспорте можно 
активировать опцию прогрессивной загрузки. Эта опция позволяет браузеру 
...
использовать одни и те же контрольные точки для разметки сайта и изображения. 
...
для конкретного изображения. Далее изложена стратегия, которой пользуюсь я при 
...
просмотра на реально существующем устройстве составляет 320 пикселей у iPhone с 
дисплеем не Retina в портретной ориентации. (Устройства с меньшей областью 
просмотра существуют и будут существовать - в виде смарт-часов, например - но 
вряд ли их будут использовать для просмотра вебсайтов). На сайте 
...
подготовить, должно иметь разрешение 290 × 183 пикселей. С помощью графического 
редактора изменяем размер исходного изображения соответственно.
...
размер изображения. По мере увеличении размера начинает страдать качество 
...
После того как нашли первую контрольную точку, из исходного изображения создаём 
...
соответствующих изображений. Для дисплеев Retina нужно создать дополнительный 
набор изображений с удвоенным разрешением. Вот полный список изображений для 
вебсайта [contfont.net][13]:
...
меры, описанные выше. Но конечно же, вебсайты редко бывают простыми и, 
...
доступных вебсайтов, ведь содержимое `alt`, описывающее изображение, необходимо пользователям, которые посещают вебсайт с помощью скринридера. У фоновых 
изображений в CSS нет тега `<img>` и соответственно атрибута `alt`, но их всё же 
можно сделать доступными для пользователей с дефектом зрения. Для этого 
...
У тега `<img>` есть еще одно полезное свойство, которое не поддерживается при 
нашем простом подходе: автоматический перерасчет размеров и сохранение пропорций 
...
эффекта и для фоновых изображений на CSS в современных браузерах. (Внимание: 
подход, описанный в следующей части статьи не работает в Internet Explorer 8 и 
старше, так как эти версии не поддерживают необходимые свойства CSS)
...
выровнять фоновое изображение в его элементе. 
...
браузеру какой должна быть высота элемента с изображением. Потому мы получим 
...
парочки незначительных дополнений.
...
Чтобы проверить как эти все компоненты сочетаются, вот код для вебсайта 
...
от контекста пользователя. (В целях лаконичности в следующем примере опущены 
вендорные префиксы).
...
служит решением для некоторых глубоких проблем отзывчивых изображений в общем. 

### Разделение стиля и содержания

CSS придумали специально для того чтобы разделить стиль и содержание. В 
современном вебе HTML-страницы несут в себе всё содержание вебсайта, а таблицы 
стилей влияют только на представление этого содержания. Таблицы стилей не должны 
определять содержание вебсайта. Спору нет, принцип разделения стиля и содержания 
...
Добавление главных изображений вебсайта с помощью CSS - это уже намного 
серьезней. Когда главный контент вебсайта добавлен с помощью CSS вместо HTML, 
становится труднее искать и справлять возникающие неисправности, такой сайт 
труднее поддерживать и возникают препятствия для систем, которые будут пытаться 
его парсить в автоматическом режиме. 
...
Как отмечено выше, улучшения, добавленные для масштабирования изображений, не 
будут работать в Internet Explorer 8 и старше. Соответствующие стандартам 
подходы вроде использования `srcset` возможно и совместимы со старыми версиями 
...
Наверное самой ключевой проблемой этого подхода является проблема собственно 
...
подход уместен. Представьте, например, что пользователь ноутбука подключается к 
интернету через точку доступа Wi-Fi на смартфоне.
...
устройства с высокой пиксельной плотностью и отзывчивый вебсайт будет 
загружаться с большими изображениями с высоким разрешением. Изображения похуже 
выглядели бы плохо на экране у этого пользователя, но изображения хорошего 
качества через бездротовое соединение могут грузиться долго. Они могут 
значительно повлиять на длительность загрузки сайта и в результате будут стоить 
нашему пользователю реальных денег за использование бездротового подключения. 
...
пока остаётся лучшим доступным нам подходом. 
